 ██████╗██╗      █████╗ ███████╗██╗  ██╗    ██████╗ ███████╗██████╗ ██████╗ ███████╗████████╗██╗   ██╗ █████╗ ████████╗██╗ ██████╗ ███╗   ██╗
██╔════╝██║     ██╔══██╗██╔════╝██║  ██║    ██╔══██╗██╔════╝██╔══██╗██╔══██╗██╔════╝╚══██╔══╝██║   ██║██╔══██╗╚══██╔══╝██║██╔═══██╗████╗  ██║
██║     ██║     ███████║███████╗███████║    ██████╔╝█████╗  ██████╔╝██████╔╝█████╗     ██║   ██║   ██║███████║   ██║   ██║██║   ██║██╔██╗ ██║
██║     ██║     ██╔══██║╚════██║██╔══██║    ██╔═══╝ ██╔══╝  ██╔══██╗██╔═══╝ ██╔══╝     ██║   ██║   ██║██╔══██║   ██║   ██║██║   ██║██║╚██╗██║
╚██████╗███████╗██║  ██║███████║██║  ██║    ██║     ███████╗██║  ██║██║     ███████╗   ██║   ╚██████╔╝██║  ██║   ██║   ██║╚██████╔╝██║ ╚████║
 ╚═════╝╚══════╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝    ╚═╝     ╚══════╝╚═╝  ╚═╝╚═╝     ╚══════╝   ╚═╝    ╚═════╝ ╚═╝  ╚═╝   ╚═╝   ╚═╝ ╚═════╝ ╚═╝  ╚═══╝
                                                                                                                                             
PROJECT TITLE: Clash Perpetuation
PURPOSE OF PROJECT: Informatik Project 10n
VERSION or DATE: 1.0.0
HOW TO START THIS PROJECT: Open project.greenfoot
AUTHORS: Tom Kuttler, Robert Cockshott
USER INSTRUCTIONS: Start the game and follow the tutorial.

------------------------------------------------------------------------
Arbeitsteilung:
Map design: Robert
Win/Lose Screen: Robert
Start Screen: Robert, Tom
Rest: Robert, Tom

------------------------------------------------------------------------
CREDITS:

Spritesheet Generator (Characters):
https://sanderfrenken.github.io/Universal-LPC-Spritesheet-Character-Generator/#

Items:
https://i.redd.it/3uo1s690gcb31.png

Font:
https://www.dafont.com/de/alagard.font

Win screen: 
https://www.istockphoto.com/de/vektor/pixel-kunst-ui-design-mit-outdoor-landschaft-hintergrund-gm1272570848-374798277
Lose screen:
https://www.istockphoto.com/de/vektor/pixel-kunst-design-mit-outdoor-landschaft-hintergrund-gm1272570902-374798322

Liberated Pixel Cup Assets:

OpenGameArt.org automatically generated credits files:

THIS FILE IS FOR INFORMATIONAL PURPOSES ONLY.  NO WARRANTY OF ANY KIND IS EXPRESSED OR IMPLIED, INCLUDING BUT NOT LIMITED TO THE WARRANTIES OF MERCHANTABILITY, FITNESS FOR A PARTICULAR PURPOSE, AND NONINFRINGEMENT. IN NO EVENT SHALL OPENGAMEART.ORG BE LIABLE FOR ANY CLAIM, DAMAGES, OR OTHER LIABILITY, WHETHER IN AN ACTION OF CONTRACT, TORT, OR OTHERWISE, ARISING FROM, OUT OF, OR IN CONNECTION WITH THE USE OF THIS FILE OR THE INFORMATION IN IT.

----------------------------------------

Title:
    Liberated Pixel Cup (LPC) Base Assets (sprites & map tiles)

Author:
    Multiple

URL:
    https://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Images are copyright their respective authors, as listed in CREDITS.TXT.  

File(s):
    * LPC Base Assets.zip

----------------------------------------

Title:
    Tiled Terrains

Author:
    Multiple

URL:
    https://opengameart.org/content/tiled-terrains

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Read the Attribution.txt

File(s):
    * LPC_Terrain.zip

----------------------------------------

Title:
    [LPC] Farming tilesets, magic animations and UI elements

Author:
    daneeklu

URL:
    https://opengameart.org/content/lpc-farming-tilesets-magic-animations-and-ui-elements

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please attribute creator as Daniel Eddeland.

Please include a link to opengameart.org as well (preferably to the submission for this art package) if you use the art.


File(s):
    * submission_daneeklu.zip

----------------------------------------

Title:
    [LPC] A shoot&#039;em up complete graphic kit

Author:
    Johann C

URL:
    https://opengameart.org/content/lpc-a-shootem-up-complete-graphic-kit

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * shootemup.tar_.gz

----------------------------------------

Title:
    Extended LPC Magic pack

Author:
    daneeklu

URL:
    https://opengameart.org/content/extended-lpc-magic-pack

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute creator as Daniel Eddeland. Please include a link to opengameart.org as well (preferably to the submission for this art package) if you use the art.

File(s):
    * magic_pack.zip
    * snake_right.png
    * turtleshell_right.png
    * magic_icons.png

----------------------------------------

Title:
    [LPC] Pennomi&#039;s UI Elements

Author:
    pennomi

Collaborators:
    Buch, cemkalyoncu

URL:
    https://opengameart.org/content/lpc-pennomis-ui-elements

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please attribute all collaborators and include a link back to OpenGameArt.org

File(s):
    * UI_ELEMENTS.zip
    * expansion_pack.zip

----------------------------------------

Title:
    [LPC] caeles&#039; art

Author:
    caeles

URL:
    https://opengameart.org/content/lpc-caeles-art

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * caeles.tar_.gz

----------------------------------------

Title:
    [LPC] Skorpio&#039;s SciFi Sprite Pack

Author:
    Skorpio

URL:
    https://opengameart.org/content/lpc-skorpios-scifi-sprite-pack

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * Skorpios_Sprite_Pack.zip

----------------------------------------

Title:
    [LPC] Maskman

Author:
    Nushio

URL:
    https://opengameart.org/content/lpc-maskman

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * maskman.zip

----------------------------------------

Title:
    [LPC] Base Character Expressions

Author:
    pennomi

Collaborators:
    laetissima

URL:
    https://opengameart.org/content/lpc-base-character-expressions

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * accessories.xcf

----------------------------------------

Title:
    [LPC] Female Orc/Ogre/Goblin/Troll (base walkcycle)

Author:
    madmarcel

URL:
    https://opengameart.org/content/lpc-female-orcogregoblintroll-base-walkcycle

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

File(s):
    * female_orcwalkcycle.xcf
    * orc_piratess.png
    * BODY_FEMALE_ORC.png

----------------------------------------

Title:
    [LPC] Sara

Author:
    William.Thompsonj

Collaborators:
    Redshrike, bart

URL:
    https://opengameart.org/content/lpc-sara

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute Stephen "Redshrike" Challener as graphic artist and William.Thompsonj as contributor. Attribute Mandi Paugh as the original artist of Sara and creator of the OGA mascot. You must include a link to the OGA homepage, per original Sara entry instructions. Please include a link to this entry also if possible.

File(s):
    * LPC_Sara.zip

----------------------------------------

Title:
    Four characters:  My LPC entries

Author:
    Redshrike

URL:
    https://opengameart.org/content/four-characters-my-lpc-entries

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribution to Stephen "Redshrike" Challener and a link to www.OpenGameArt.org
For Sinbad, attribution to Zi Ye for the design.

Copyright/Attribution Notice:
    Stephen Challener (Redshrike) and Zi Ye, hosted by OpenGameArt.org

File(s):
    * mage fall down sheet copy.png
    * Mage spell casting sheet copy.png
    * mage walking poses sheet copy.png
    * baldricfrontwalksheet copy.png
    * baldricSlashSheet.png
    * baldricfallsheet copy.png
    * baldricSpellSheet copy.png
    * unfinishedchars1.PNG

----------------------------------------

Title:
    [LPC] 2 Characters

Author:
    pennomi

Collaborators:
    laetissima, makrohn

URL:
    https://opengameart.org/content/lpc-2-characters

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute pennomi, laetissima and makrohn. Remember to also attribute the LPC base sprite artists, since this is a derivative work of those as well!

File(s):
    * female_spritesheet.xcf
    * male_spritesheet.xcf
    * irishdress.png

----------------------------------------

Title:
    [LPC] Goblin

Author:
    William.Thompsonj

Collaborators:
    Redshrike

URL:
    https://opengameart.org/content/lpc-goblin

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    Attribute Stephen "Redshrike" Challener as graphic artist and William.Thompsonj as contributor. If reasonable link to this page or the OGA homepage.

File(s):
    * goblin.png
    * sword.png
    * goblinsword.png
    * goblin sheeting.psd

----------------------------------------

Title:
    Animated Coins

Author:
    Clint Bellanger

URL:
    https://opengameart.org/content/animated-coins

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Clint Bellanger for Liberated Pixel Art

File(s):
    * coin_gold.png
    * coin_silver.png
    * coin_copper.png
    * gold_coin.xcf
    * silver_coin.xcf
    * copper_coin.xcf
    * coin.blend

----------------------------------------

Title:
    [LPC] Spider

Author:
    William.Thompsonj

Collaborators:
    Redshrike

URL:
    https://opengameart.org/content/lpc-spider

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute Stephen "Redshrike" Challener as graphic artist and William.Thompsonj as contributor. If reasonable link to this page or the OGA homepage.

File(s):
    * LPC_Spiders.zip

----------------------------------------

Title:
    [LPC] Interface Items Contribution by Tempest in the Aether

Author:
    Tempest in the Aether and Ryzom Core

URL:
    https://opengameart.org/content/lpc-interface-items-contribution-by-tempest-in-the-aether

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Credit Tempest in the Aether and Ryzom Core and post derivative works on opengameart.org

File(s):
    * Tempest_In_The_Aether_Interface.7z
    * Tempest_In_The_Aether_Interface.zip

----------------------------------------

Title:
    Road tile set.

Author:
    Sir Spummington

URL:
    https://opengameart.org/content/road-tile-set

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute to Connor Sherson (cppsherson@gmail.com)

File(s):
    * roadpage.xcf

----------------------------------------

Title:
    Chainlink floor tiles

Author:
    Sir Spummington

URL:
    https://opengameart.org/content/chainlink-floor-tiles

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute to Connor Sherson (cppsherson@gmail.com) 

File(s):
    * chainlinkpagerusty.xcf

----------------------------------------

Title:
    Interior Tiles

Author:
    Sir Spummington

URL:
    https://opengameart.org/content/interior-tiles

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute to Connor Sherson (cppsheron@gmail.com)

File(s):
    * interior2.xcf

----------------------------------------

Title:
    [LPC] Medieval fantasy character sprites

Author:
    wulax

URL:
    https://opengameart.org/content/lpc-medieval-fantasy-character-sprites

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    I waive the DRM-limitation clause in the Creative Commons license for this art collection, allowing their free use on platforms like iOS or Steam provided you give credit as specified.

File(s):
    * lpc_entry.zip
    * expansion_pack-0.04.zip

----------------------------------------

Title:
    [LPC] Colorful Sand + Deep Water!

Author:
    Nushio

Collaborators:
    daneeklu, cemkalyoncu

URL:
    https://opengameart.org/content/lpc-colorful-sand-deep-water

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Link back to the OpenGameArt website, and mention the LPC. 

File(s):
    * snow.png
    * snowwater.png
    * snowice.png
    * snowwateralt.png
    * coldwater.png
    * coldwatergrass.png
    * ice.png
    * icegrass.png
    * Winter tiles.7z
    * redsandwater.png
    * redsand.png
    * deepwater.png
    * deepwater2.png

----------------------------------------

Title:
    LPC beetle

Author:
    Redshrike

URL:
    https://opengameart.org/content/lpc-beetle

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Stephen "Redshrike" Challener, plus a link back to opengameart.org

Copyright/Attribution Notice:
    Stephen Challener (Redshrike), hosted by OpenGameArt.org

File(s):
    * beetle5.PNG
    * mace.PNG

----------------------------------------

Title:
    LPC in battle RPG sprites

Author:
    Redshrike

URL:
    https://opengameart.org/content/lpc-in-battle-rpg-sprites

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Stephen Challener (Redshrike), Matthew Nash, Tunicate, Ben "Cookiez" Potter (with a link to this page: http://cookiez999.deviantart.com/), Charles Gabriel, as noted in description.  Also include a link to OpenGameArt.org

File(s):
    * in battle sprites 1.PNG

----------------------------------------

Title:
    LPC Sage

Author:
    Yamilian

URL:
    https://opengameart.org/content/lpc-sage

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * sage.xcf

----------------------------------------

Title:
    3-form RPG boss: "Harlequin Epicycle."

Author:
    Redshrike

URL:
    https://opengameart.org/content/3-form-rpg-boss-harlequin-epicycle

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    Stephen Challener (Redshrike), hosted by OpenGameArt.org

File(s):
    * threeformsPJ2.png

----------------------------------------

Title:
    Bomb Explosion Animation

Author:
    qubodup

Collaborators:
    shimobayashi

URL:
    https://opengameart.org/content/bomb-explosion-animation

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Blood _ Copyright 2011 by shimobayashi <http://opengameart.org/content/teqq-princess-image-materials> Color edit to look like explosion Copyright 2012 by qubodup <http://opengameart.org/users/qubodup> License: CCBY3 or later <http://creativecommons.org/licenses/by/3.0/>

File(s):
    * explosion-xcf-bz2.zip
    * explosion.png

----------------------------------------

Title:
    [LPC] LifeBars!

Author:
    Nushio

Collaborators:
    pennomi

URL:
    https://opengameart.org/content/lpc-lifebars

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * rad-rainbow-lifebar.xcf
    * red-cherry-lifebar.xcf
    * rad-rainbow-lifebar.png
    * red-cherry-lifebar.png

----------------------------------------

Title:
    [LPC] Guido Bos entries cut up

Author:
    Guido Bos

URL:
    https://opengameart.org/content/lpc-guido-bos-entries-cut-up

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please use the LPC attribution instructions for this work

File(s):
    * miscellaneous.png
    * town_buildings.png
    * housey.png
    * flowers.png

----------------------------------------

Title:
    LPC Heroine

Author:
    Yamilian

URL:
    https://opengameart.org/content/lpc-heroine

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * lidia.xcf

----------------------------------------

Title:
    Alternate LPC character sprites -- George

Author:
    sheep

URL:
    https://opengameart.org/content/alternate-lpc-character-sprites-george

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * george.png

----------------------------------------

Title:
    One more LPC alternate character

Author:
    Radomir Dopieralski

URL:
    https://opengameart.org/content/one-more-lpc-alternate-character

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * betty.png

----------------------------------------

Title:
    Generic 8-bit JRPG Soundtrack

Author:
    AVGVSTA

URL:
    https://opengameart.org/content/generic-8-bit-jrpg-soundtrack

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    AVGVSTA

File(s):
    * JRPG OST (Rev 2).zip
    * Rev 2 Source.zip

----------------------------------------

Title:
    LPC Weapons: two bows, a spear and a trident

Author:
    daneeklu

Collaborators:
    wulax, gr3yh47

URL:
    https://opengameart.org/content/lpc-weapons-two-bows-a-spear-and-a-trident

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

File(s):
    * dragonspear.png
    * greatbow.png
    * trident.png
    * mace_sheet.png
    * sabre_sheet.png
    * recurvebow_sheet.png

----------------------------------------

Title:
    Lightning Cloud

Author:
    dorkster

URL:
    https://opengameart.org/content/lightning-cloud

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

File(s):
    * lightning.png

----------------------------------------

Title:
    LPC Heroine 2

Author:
    Yamilian

URL:
    https://opengameart.org/content/lpc-heroine-2

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * shura.xcf

----------------------------------------

Title:
    [LPC] Obok and Gossip art additions and modifications

Author:
    caeles

URL:
    https://opengameart.org/content/lpc-obok-and-gossip-art-additions-and-modifications

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * blossom.png
    * fadeout.png
    * life.png
    * speechbubble.png
    * wavingleaves.png
    * bee.png
    * bee_shadow.png
    * birdbath.png
    * buckets_wo_shadow.png
    * frame.png
    * house.png
    * platformtree.png
    * signpost.png
    * vehicle_all.png
    * wet_princess.png
    * platformtree.xcf
    * vehicle_all.xcf
    * vehicle_parts.xcf

----------------------------------------

Title:
    LPC ladies

Author:
    makrohn

URL:
    https://opengameart.org/content/lpc-ladies

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Walkcycle, Hurt, Slash, and Spellcast animations by Stephen Challener (AKA Redshrike), 
Shooting and Thrusting adapted from animations by Johannes Sjölund, 
Male and female orc heads by MadMarcel and based on Stephen Challener&#039;s Sinbad, 
Tanned, Tanned2, and Dark colors by MadMarcel, 
other credit to Matthew Krohn

File(s):
    * lpcfemalelight.png
    * lpcfemaletanned.png
    * lpcfemaletanned2.png
    * lpcfemaledark.png
    * lpcfemaleorc.png

----------------------------------------

Title:
    LPC Dress in Combat Poses

Author:
    makrohn

URL:
    https://opengameart.org/content/lpc-dress-in-combat-poses

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Walkcycle poses by Lanea Zimmerman (AKA Sharm), other poses by Matthew Krohn

File(s):
    * lpcfemaleoverskirt.png
    * lpcfemaleunderdress.png
    * lpcfemalevest.png

----------------------------------------

Title:
    LPC Male Sheets

Author:
    makrohn

URL:
    https://opengameart.org/content/lpc-male-sheets

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Walkcycle, Hurt, Slash, and Spellcast animations by Stephen Challener (AKA Redshrike), 
Shooting and Thrusting animations by Johannes Sjölund, 
Male and female orc heads by MadMarcel and based on Stephen Challener&#039;s Sinbad, 
Tanned, Tanned2, and Dark colors by MadMarcel, 
other credit to Matthew Krohn

File(s):
    * darkmale.png
    * tanned2male.png
    * tannedmale.png
    * maleorcfullsheet.png

----------------------------------------

Title:
    LPC Combat Armor for women

Author:
    makrohn

URL:
    https://opengameart.org/content/lpc-combat-armor-for-women

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Adapted by Matthew Krohn from art created by Johannes Sjölund

File(s):
    * lpcfemalechainmail.png
    * lpcfemalechaintabard.png
    * lpcfemaleleatherbracers.png
    * lpcfemaleleathershoulders.png
    * lpcfemaleleathertorso.png
    * lpcfemaleplateboots.png
    * lpcfemaleplategloves.png
    * lpcfemaleplategreaves.png
    * lpcfemaleplatehelmet.png
    * lpcfemaleplatemail.png
    * lpcfemaleplatesaulders.png

----------------------------------------

Title:
    LPC Tile Atlas

Author:
    See Attribution.txt

URL:
    https://opengameart.org/content/lpc-tile-atlas

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    The zip contains a nifty attribution file so you can drop it and forget.

File(s):
    * Atlas.zip

----------------------------------------

Title:
    Shadowless LPC food

Author:
    caeles

URL:
    https://opengameart.org/content/shadowless-lpc-food

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * farming_fishing.png
    * Vegies.png

----------------------------------------

Title:
    Helmetfish (Part 1?)

Author:
    Redshrike

URL:
    https://opengameart.org/content/helmetfish-part-1

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    Stephen Challener (Redshrike), hosted by OpenGameArt.org

File(s):
    * helmetfishPNG.png

----------------------------------------

Title:
    LPC Fireplace

Author:
    Sharm

URL:
    https://opengameart.org/content/lpc-fireplace

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please credit Lanea Zimmerman

File(s):
    * fireplace.png
    * fireplace.gif

----------------------------------------

Title:
    LPC Wands

Author:
    makrohn

URL:
    https://opengameart.org/content/lpc-wands

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Matthew Krohn

File(s):
    * lpcfemalewandsteel.png
    * lpcfemalewandwood.png
    * lpcmalewandsteel.png
    * lpcmalewandwood.png

----------------------------------------

Title:
    [Unsealed] TubaTurtle

Author:
    Ben Potter

URL:
    https://opengameart.org/content/unsealed-tubaturtle

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    A link to Cookiez&#039; DeviantArt and/or this OGA Entry. 

File(s):
    * turtle.png

----------------------------------------

Title:
    [Unsealed] Ghost

Author:
    Ben Potter

URL:
    https://opengameart.org/content/unsealed-ghost

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Credit Cookiez, the LPC Contest too and link to this entry. 

File(s):
    * ghost.png

----------------------------------------

Title:
    [Unsealed] Terrex

Author:
    Wyverii

URL:
    https://opengameart.org/content/unsealed-terrex

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    A link to this entry and/or http://unsealed.k3rnel.net as well as credit to Wyverii

File(s):
    * terrex.png

----------------------------------------

Title:
    [Unsealed] Shura: Battle Gfx

Author:
    Laura &#039;Yamilian&#039; Armendariz

Collaborators:
    Yamilian

URL:
    https://opengameart.org/content/unsealed-shura-battle-gfx

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    The author is Yamilian, a link to this entry and/or http://unsealed.k3rnel.net would be fine. 

File(s):
    * shura.png

----------------------------------------

Title:
    Livestock

Author:
    brooss

URL:
    https://opengameart.org/content/livestock-0

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

File(s):
    * livestock.zip

----------------------------------------

Title:
    LPC style farm animals

Author:
    daneeklu

URL:
    https://opengameart.org/content/lpc-style-farm-animals

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute to Daniel Eddeland, and link to opengameart (preferably this submission).

File(s):
    * chicken_eat.png
    * chicken_walk.png
    * chicken_shadow.png
    * cow_eat.png
    * cow_walk.png
    * cow_shadow.png
    * llama_eat.png
    * llama_walk.png
    * llama_shadow.png
    * pig_eat.png
    * pig_walk.png
    * sheep_eat.png
    * sheep_walk.png

----------------------------------------

Title:
    [Unsealed] Lidia: Battle Gfx

Author:
    Ben Potter

URL:
    https://opengameart.org/content/unsealed-lidia-battle-gfx

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Credit Ben &#039;Cookiez&#039; Potter and link to this entry if possible

File(s):
    * lidia.png

----------------------------------------

Title:
    Elemental absorbtion animation

Author:
    Sven Stamm

URL:
    https://opengameart.org/content/elemental-absorbtion-animation

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

File(s):
    * Sog.png

----------------------------------------

Title:
    Misc 32x32 Tiles

Author:
    Matthew Nash

URL:
    https://opengameart.org/content/misc-32x32-tiles

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please include credit to opengameart.org

File(s):
    * misc.png

----------------------------------------

Title:
    LPC Tile Atlas2

Author:
    See Attribution.txt

URL:
    https://opengameart.org/content/lpc-tile-atlas2

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    The zip contains a nifty attribution file so you can drop it and forget.

File(s):
    * Atlas2.zip

----------------------------------------

Title:
    [LPC] full plate golden armor

Author:
    Shaun Williams

URL:
    https://opengameart.org/content/lpc-full-plate-golden-armor

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

File(s):
    * BODY_male_Epic_armors_Golden.png

----------------------------------------

Title:
    [LPC] Tabletop RPG + pizza

Author:
    pennomi

URL:
    https://opengameart.org/content/lpc-tabletop-rpg-pizza

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * dnd.xcf

----------------------------------------

Title:
    LPC Plasmaball

Author:
    makrohn

URL:
    https://opengameart.org/content/lpc-plasmaball

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Matthew Krohn, based upon original design by Thane Brimhall and Geni

File(s):
    * plasmaballlpc.png

----------------------------------------

Title:
    Castle Tiles for RPG&#039;s

Author:
    Hyptosis, Zabin, Daniel Cook

URL:
    https://opengameart.org/content/castle-tiles-for-rpgs

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Zabin, Hyptosis, and Danial Cook. https://opengameart.org/content/castle-tiles-for-rpgs

File(s):
    * Castle2.png

----------------------------------------

Title:
    [LPC] Skeleton

Author:
    wulax

URL:
    https://opengameart.org/content/lpc-skeleton

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * skeleton.png

----------------------------------------

Title:
    LPC Full Plate Golden Armor

Author:
    makrohn

URL:
    https://opengameart.org/content/lpc-full-plate-golden-armor-0

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Matthew Krohn, adapted from the work of Johannes Sjölund and Shaun Williams

File(s):
    * lpcfemalegoldenboots.png
    * lpcfemalegoldengloves.png
    * lpcfemalegoldengreaves.png
    * lpcfemalegoldenhelm.png
    * lpcfemalegoldenplate.png
    * lpcfemalegoldenspaulders.png
    * lpcmalegoldenboots.png
    * lpcmalegoldengloves.png
    * lpcmalegoldengreaves.png
    * lpcmalegoldenhelm.png
    * lpcmalegoldenplate.png
    * lpcmalegoldenspaulders.png
    * lpcgoldenarmor.tar.gz

----------------------------------------

Title:
    LPC terrain extension

Author:
    caeles

URL:
    https://opengameart.org/content/lpc-terrain-extension

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    For detailed copyright information see the file COPYRIGHT in the archive. The authors are Cem Kalyoncu, Connor Sherson, Daniel Eddeland, Johann Charlot, Jonas Klinger, Juan Rodriguez, Lanea Zimmerman, Mark Weyer, and Skyler Robert Colladay.

File(s):
    * lpc_terrain.tar

----------------------------------------

Title:
    RPG Tiles: Cobble stone paths & town objects

Author:
    Zabin, Daneeklu, Jetrel, Hyptosis, Redshrike, Bertram

Collaborators:
    daneeklu, Jetrel, Hyptosis, Redshrike, Bertram

URL:
    https://opengameart.org/content/rpg-tiles-cobble-stone-paths-town-objects

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Credit goes to:  Zabin, Daneeklu, Jetrel, Hyptosis, Redshrike, Bertram.  A link back to this page "http://opengameart.org/content/rpg-tiles-cobble-stone-paths-town-objects to see who did what"

File(s):
    * PathAndObjects.png
    * Artistinfo2.png

----------------------------------------

Title:
    Camp Fire Animation for RPGs (finished)

Author:
    Zabin and Jetrel

URL:
    https://opengameart.org/content/camp-fire-animation-for-rpgs-finished

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Credit goes to Zabin and Jetrel from OGA

File(s):
    * CampFire.png
    * NewCampFire.png
    * NewerCampFire.png
    * CampFireFinished.png

----------------------------------------

Title:
    Ponytail and Plain hairstyles

Author:
    Joe White

URL:
    https://opengameart.org/content/ponytail-and-plain-hairstyles

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * ponytail-and-plain.zip

----------------------------------------

Title:
    Angled tiles for LPC&#039;s cement, brackish and dungeon tilesets

Author:
    Lanea Zimmerman (AKA Sharm)

URL:
    https://opengameart.org/content/angled-tiles-for-lpcs-cement-brackish-and-dungeon-tilesets

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Original by Lanea Zimmerman (AKA Sharm)
Modifications by Xenodora

File(s):
    * cement-angle.png
    * brackish-angle.png
    * dungeon-angle.png

----------------------------------------

Title:
    LPC style wood bridges and steel flooring

Author:
    Xenodora

URL:
    https://opengameart.org/content/lpc-style-wood-bridges-and-steel-flooring

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Put Xenodora in the appropriate section of the credits.

File(s):
    * bridge-wood-angled.png
    * bridge-wood-square.png
    * floor-steel.png
    * railing-wood-16x32.png
    * railing-steel-16x32.png

----------------------------------------

Title:
    Knife Walk

Author:
    XOR

URL:
    https://opengameart.org/content/knife-walk

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

File(s):
    * knife_walk.xcf

----------------------------------------

Title:
    Stone Bridge tiles 32x32

Author:
    Reemax

URL:
    https://opengameart.org/content/stone-bridge-tiles-32x32

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Stone bridge tiles made by Tuomo Untinen

File(s):
    * stone-bridge.png
    * stone-bridge2.png

----------------------------------------

Title:
    LPC Space Base Tube Passage

Author:
    Xenodora

URL:
    https://opengameart.org/content/lpc-space-base-tube-passage

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Put Xenodora in the appropriate section of the credits.

File(s):
    * space-base-1.png
    * space-base-2.png

----------------------------------------

Title:
    Football sprite (based on LPC set)

Author:
    asalinasci

URL:
    https://opengameart.org/content/football-sprite-based-on-lpc-set

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    www.vedoque.com

File(s):
    * hombre-lpc2.xcf

----------------------------------------

Title:
    LPC Mine Carts and Tracks

Author:
    Xenodora

URL:
    https://opengameart.org/content/lpc-mine-carts-and-tracks

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Put Xenodora in the appropriate section of the credits.

File(s):
    * lpc-mine-cart-tracks.png
    * lpc-mine-cart-tracks-rails-only.png
    * lpc-mine-cart-tracks-sleepers-only.png
    * lpc-mine-cart-carts.png
    * lpc-mine-cart.xcf.gz
    * lpc-mine-cart-tracks-angles.png
    * lpc-mine-cart-tracks-angles-rails-only.png
    * lpc-mine-cart-tracks-angles-sleepers-only.png
    * lpc-mine-cart-carts-angles.png
    * lpc-mine-cart-angles.xcf.gz
    * lpc-mine-cart-tracks-sample.zip

----------------------------------------

Title:
    [LPC] Imp

Author:
    William.Thompsonj

Collaborators:
    Redshrike

URL:
    https://opengameart.org/content/lpc-imp

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Link my profile (if reasonable), credit Redshrike as graphic artist, and credit me as contributor (I did pay to have it made after all...)

File(s):
    * LPC imp.zip

----------------------------------------

Title:
    [LPC] Animated water and fire

Author:
    Sharm

Collaborators:
    Curt

URL:
    https://opengameart.org/content/lpc-animated-water-and-fire

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Sharm and Curt
this web address

File(s):
    * WaterAndFire.png

----------------------------------------

Title:
    [LPC] Leaf Recolor

Author:
    William.Thompsonj

URL:
    https://opengameart.org/content/lpc-leaf-recolor

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Credit file included. You can link my profile if you want.

File(s):
    * lpc leaf recolors.zip

----------------------------------------

Title:
    [LPC] Tree Recolors

Author:
    William.Thompsonj

Collaborators:
    C.Nilsson

URL:
    https://opengameart.org/content/lpc-tree-recolors

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Credit C. Nilsson for his original trees. If you want to credit me please link my profile. Credits file is provided but does not contain my information.

File(s):
    * green trees.png
    * brown trees.png
    * autumn 2.png
    * autumn.png
    * cherry_blossom_trees.png
    * yellow.png
    * credits.zip
    * brown trees 2.png
    * yellow 2.png
    * autumn 3.png
    * blue trees.png

----------------------------------------

Title:
    [LPC] Flames

Author:
    William.Thompsonj

Collaborators:
    Sharm

URL:
    https://opengameart.org/content/lpc-flames

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute Sharm as graphic artist. If you want to link my profile you can.

File(s):
    * flames.png
    * flames.gif
    * flames.zip

----------------------------------------

Title:
    [LPC] Plant Repack

Author:
    various

Collaborators:
    pennomi, laetissima, C.Nilsson, Sharm, Johann C, Reemax

URL:
    https://opengameart.org/content/lpc-plant-repack

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please see the credits file. If I missed someone or wrongly included someone please let me know, I&#039;ll fix it.

File(s):
    * credits.zip
    * plant repack.png

----------------------------------------

Title:
    [LPC] Animated Torch

Author:
    William.Thompsonj

Collaborators:
    HughSpectrum, Sharm

URL:
    https://opengameart.org/content/lpc-animated-torch

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Link this page or my profile, Hugh Spectrum and Sharm as graphic artists.

File(s):
    * animated_torch.gif
    * animated_torch.png

----------------------------------------

Title:
    [LPC] Adobe Building Set

Author:
    Sharm

Collaborators:
    Sharm

URL:
    https://opengameart.org/content/lpc-adobe-building-set

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please link this page (if reasonable), attribute Sharm as graphic artist, and me as contributor (since I did pay out of the goodness of my heart to have this set made after all...). File called credit.txt included for convenience.

File(s):
    * adobe building set.zip

----------------------------------------

Title:
    [LPC] Arabic Elements

Author:
    Sharm

Collaborators:
    Sharm

URL:
    https://opengameart.org/content/lpc-arabic-elements

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please link this page (if reasonable), attribute Sharm as graphic artist, and me as contributor.

File(s):
    * arabic set.zip

----------------------------------------

Title:
    LPC Style Well

Author:
    Xenodora

URL:
    https://opengameart.org/content/lpc-style-well

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Credit Xenodora for the well.
Credit Lanea Zimmerman for the buckets.


File(s):
    * lpc-well.png
    * lpc-well.xcf.gz

----------------------------------------

Title:
    [LPC] Short Sword

Author:
    William.Thompsonj

Collaborators:
    wulax

URL:
    https://opengameart.org/content/lpc-short-sword

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Link this page (if reasonable), credit William.Thompsonj, and wulax.

File(s):
    * LPC Short Sword.zip

----------------------------------------

Title:
    [LPC] Castle Lights Repack

Author:
    HughSpectrum

URL:
    https://opengameart.org/content/lpc-castle-lights-repack

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Daniel Armstrong (AKA HughSpectrum), see the original work in the LPC assets for specific attribution instructions http://lpc.opengameart.org/static/lpc-style-guide/index.html

File(s):
    * lights.png

----------------------------------------

Title:
    [LPC] Dungeon Elements

Author:
    Sharm

Collaborators:
    Sharm

URL:
    https://opengameart.org/content/lpc-dungeon-elements

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please link this page (if reasonable), attribute Sharm as graphic artist, and me as contributor. credit.txt included for convenience.

File(s):
    * dungeon.zip

----------------------------------------

Title:
    [LPC] Terrain Repack

Author:
    vaious

URL:
    https://opengameart.org/content/lpc-terrain-repack

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please see the text file copied from the LPC Tile Atlas. I take no credit for any of the graphics included here, I only repacked them using Tiled.

File(s):
    * terrain.zip

----------------------------------------

Title:
    LPC Animated Castle Doors

Author:
    Daniel Armstrong (AKA HughSpectrum)

URL:
    https://opengameart.org/content/lpc-animated-castle-doors

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Original castle door by Daniel Armstrong (AKA HughSpectrum).

File(s):
    * lpc-doors-animated-2.png

----------------------------------------

Title:
    LPC Animated Doors

Author:
    Lanea Zimmerman (AKA Sharm)

URL:
    https://opengameart.org/content/lpc-animated-doors

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Original wood and dungeon doors by Lanea Zimmerman (AKA Sharm).


File(s):
    * lpc-doors-animated-1.png

----------------------------------------

Title:
    LPC: Interior Castle Tiles

Author:
    Sharm

URL:
    https://opengameart.org/content/lpc-interior-castle-tiles

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please credit Lanea Zimmerman

File(s):
    * castle.png
    * stairs.png

----------------------------------------

Title:
    Recursive Cave Entrance

Author:
    makrohn

URL:
    https://opengameart.org/content/recursive-cave-entrance

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Adapted from original by Lanea Zimmerman.

File(s):
    * cavernopening.png

----------------------------------------

Title:
    LPC girl variant 2

Author:
    sheep & Radomir Dopieralski

URL:
    https://opengameart.org/content/lpc-girl-variant-2

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Based on Radomir Dopieralski&#039;s https://opengameart.org/content/one-more-lpc-alternate-character and Sheep&#039;s https://opengameart.org/content/alternate-lpc-character-sprites-george 

File(s):
    * ada_0.png
    * dante_0.png
    * dante_1.png
    * dante-hair.png

----------------------------------------

Title:
    [LPC] Water Fountain

Author:
    Sharm

Collaborators:
    William.Thompsonj, Curt

URL:
    https://opengameart.org/content/lpc-water-fountain

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Curt, Sharm, William.Thompsonj, This Webaddress

File(s):
    * WaterFountain.png

----------------------------------------

Title:
    [LPC] Expressions

Author:
    Curt

URL:
    https://opengameart.org/content/lpc-expressions

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Curt - This web address

File(s):
    * Expressions.png

----------------------------------------

Title:
    LPC Cave Stairs

Author:
    makrohn

URL:
    https://opengameart.org/content/lpc-cave-stairs

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Adapted by Matthew Krohn from originals by Lanea Zimmerman

File(s):
    * cavesteps.png

----------------------------------------

Title:
    [LPC] Curly Hair, Elven Ears, White Cape with blue trim, and more!

Author:
    Jaidyn Reiman

Collaborators:
    makrohn, JaidynReiman

URL:
    https://opengameart.org/content/lpc-curly-hair-elven-ears-white-cape-with-blue-trim-and-more

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    See authors here: https://github.com/jrconway3/Universal-LPC-spritesheet/blob/master/AUTHORS.txt

File(s):
    * ruby-red.png
    * elvenears_light.png
    * red_orc.png
    * teal_tunic.png
    * cape_white.png
    * trimcape_whiteblue.png
    * capeclip_blue.png
    * capetie_white.png
    * maroon_sleeveless.png

----------------------------------------

Title:
    Four-level meter indicator

Author:
    whilefun

URL:
    https://opengameart.org/content/four-level-meter-indicator

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    If you want to thank me in your readme files or whatever, that&#039;s of course welcome. Not required though.

File(s):
    * spriteShieldsLevel_strip4.png

----------------------------------------

Title:
    LPC Dark Elves

Author:
    Nila122

URL:
    https://opengameart.org/content/lpc-dark-elves

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Walkcycle, Hurt, Slash, and Spellcast animations by Stephen Challener (AKA Redshrike),

Shooting and Thrusting animations by Johannes Sjölund, 

Ears by Makrohn and David Conway Jr. (AKA Jaidyn Reiman) for cleaning up the ears

File(s):
    * LPC Dark Elves.zip

----------------------------------------

Title:
    1 Hour LPC Enemy

Author:
    Redshrike

URL:
    https://opengameart.org/content/1-hour-lpc-enemy

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Stephen "Redshrike" Challener, and a link back to OpenGameArt.org

File(s):
    * 1hoursheet.PNG

----------------------------------------

Title:
    Dragon idle animation

Author:
    Scribe

Collaborators:
    Redshrike, surt

URL:
    https://opengameart.org/content/dragon-idle-animation

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please credit Redshrike and Surt for the original work. Optionally you can also credit me as either Scribe or Daniel Stephens. A link back to OGA is also always appreciated

File(s):
    * 0.png
    * 1.png
    * 2.png
    * 0 [updated].png
    * 1 [updated].png
    * 2 [updated].png

----------------------------------------

Title:
    Rock-Paper-Scissors LPC style

Author:
    caeles

URL:
    https://opengameart.org/content/rock-paper-scissors-lpc-style

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    The cast frames and the cast preview are based on work by Stephen Challener (slash animation and human bodies), Johannes Sjölund (recolours and skeleton), and Marcel van de Steeg (recolours and orc).
The other preview uses art by Bart Kelsey, Charles Sanchez, Christopher Allan Webber, Daniel Armstrong, John Sullivan, Jonathan Palecek, Joshua Gay, Lanea Zimmerman, Manuel Riecke, Rob Hawkes, and Stephen Challener

File(s):
    * rock-paper-scissors.xcf
    * rps-cast.png

----------------------------------------

Title:
    Shabby Chic Exterior Tiles

Author:
    cyanowl

URL:
    https://opengameart.org/content/shabby-chic-exterior-tiles

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

File(s):
    * ShabbyChic_Home.png

----------------------------------------

Title:
    Home Objects

Author:
    Jannax

URL:
    https://opengameart.org/content/home-objects

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Feel free to use however you want. Credit isn&#039;t necessary but much appreciated. If you do credit, simply credit me as Janna or Lilius, thanks!

File(s):
    * House Objects 1.png
    * House Objects 1 Revised.png

----------------------------------------

Title:
    Sleek Bars

Author:
    Jannax

URL:
    https://opengameart.org/content/sleek-bars

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Feel free to use however you want. Credit isn&#039;t necessary but much appreciated. If you do credit, simply credit me as Janna, thanks!

File(s):
    * SleekBars.png

----------------------------------------

Title:
    Sci-Fi Object Set

Author:
    Jannax

URL:
    https://opengameart.org/content/sci-fi-object-set

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Feel free to use however you want. Credit isn&#039;t necessary but much appreciated. If you do credit, simply credit me as Janna or Lilius, thanks!

File(s):
    * Sci-fi Obj Set 1.png

----------------------------------------

Title:
    [LPC] Wolf Animation

Author:
    William.Thompsonj

Collaborators:
    Redshrike

URL:
    https://opengameart.org/content/lpc-wolf-animation

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute Stephen "Redshrike" Challener as graphic artist and William.Thompsonj as contributor. If reasonable link back to this page or the OGA homepage.

File(s):
    * howl.png
    * wolfsheet1.png
    * wolfsheet2.png
    * wolfsheet3.png
    * wolfsheet4.png
    * wolfsheet5.png
    * wolfsheet6.png
    * wolfsheetsSource.psd

----------------------------------------

Title:
    [LPC] Sand+Rock Alt Colors

Author:
    William.Thompsonj

Collaborators:
    daneeklu

URL:
    https://opengameart.org/content/lpc-sandrock-alt-colors

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please attribute William.Thompsonj, Daniel Eddeland, and link back to this page.

File(s):
    * sand.png

----------------------------------------

Title:
    [LPC] Clock Animation

Author:
    William.Thompsonj

Collaborators:
    Sharm

URL:
    https://opengameart.org/content/lpc-clock-animation

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please credit Lanea Zimmerman (AKA Sharm) as she requested in her original post: http://opengameart.org/content/lpc-interior-castle-tiles

File(s):
    * animated_clock.png

----------------------------------------

Title:
    [LPC] House Insides

Author:
    Lanea Zimmerman (AKA Sharm)

Collaborators:
    Sharm, HughSpectrum

URL:
    https://opengameart.org/content/lpc-house-insides

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Sharm did everything except the castle light sources, those were done by HughSpectrum. Please see the LPC guidelines for attribution and licensing: http://lpc.opengameart.org/

File(s):
    * house_inside.png

----------------------------------------

Title:
    [LPC] Terrain Repack 2

Author:
    Nushio

Collaborators:
    daneeklu, cemkalyoncu

URL:
    https://opengameart.org/content/lpc-terrain-repack-2

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    See original entry here: http://opengameart.org/content/lpc-colorful-sand-deep-water

File(s):
    * terrain2.png

----------------------------------------

Title:
    [LPC] Imp 2

Author:
    William.Thompsonj

Collaborators:
    Redshrike

URL:
    https://opengameart.org/content/lpc-imp-2

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Attribute Stephen "Redshrike" Challener as graphic artist and William.Thompsonj as contributor. If reasonable link to this page or the OGA homepage.

File(s):
    * LPC Imp 2.zip

----------------------------------------

Title:
    [LPC] Flower Recolor

Author:
    William.Thompsonj

Collaborators:
    Sharm

URL:
    https://opengameart.org/content/lpc-flower-recolor

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please credit Lanea Zimmerman "Sharm" as graphic artist. If you want to credit me you can list me as a contributor.

File(s):
    * flower.png
    * flower_base.png
    * flower_only.png

----------------------------------------

Title:
    [LPC] Misc

Author:
    Lanea Zimmerman

Collaborators:
    Sharm

URL:
    https://opengameart.org/content/lpc-misc

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Please credit Lanea Zimmerman "Sharm" as graphic artist. If you want to credit me you can list me as a contributor.

File(s):
    * lpc_misc.png

----------------------------------------

Title:
    LPC Candy

Author:
    caeles

URL:
    https://opengameart.org/content/lpc-candy

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    The preview includes art by Stephen Challener.

The other files make no attribution requirement. If you want to credit me out of courtesy, my name is Mark Weyer. If you want to be extra courtous, I appreciate when you tell me how you use the art.


File(s):
    * candy.tar

----------------------------------------

Title:
    LPC More Water Transitions

Author:
    billknye

URL:
    https://opengameart.org/content/lpc-more-water-transitions

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    Based on: http://opengameart.org/content/lpc-terrain-extension

File(s):
    * coldwatericetransitions.zip

----------------------------------------

Title:
    Electrical disintegration animation

Author:
    superjoe

URL:
    https://opengameart.org/content/electrical-disintegration-animation

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

File(s):
    * disintegrate-fx.zip

----------------------------------------

Title:
    Dorver Monster

Author:
    Manveru

URL:
    https://opengameart.org/content/dorver-monster

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    Please mention Manveru and Dorimen as the authors. And possibly provide a link to this page. Enjoy ^^

File(s):
    * dorver.7z

----------------------------------------

Title:
    Drakes and lizardfolk

Author:
    Nila122

URL:
    https://opengameart.org/content/drakes-and-lizardfolk

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    Original Walkcycle, Hurt, Slash, and Spellcast animations by Stephen Challener (AKA Redshrike) and Shooting and Thrusting animations by Johannes Sjölund

File(s):
    * Reptile.zip

----------------------------------------

Title:
    Selection Tile Set 32x32

Author:
    hansonry

URL:
    https://opengameart.org/content/selection-tile-set-32x32

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

File(s):
    * field.zip

----------------------------------------

Title:
    Bosses and monsters spritesheets (Ars Notoria)

Author:
    Balmer

Collaborators:
    Redshrike

URL:
    https://opengameart.org/content/bosses-and-monsters-spritesheets-ars-notoria

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

Copyright/Attribution Notice:
    Attibution to Stephen Challener (Redshrike) for the original sprites and a link to OpenGameArt.org

File(s):
    * spritesheets.zip

----------------------------------------

Title:
    LPC clothes and hair

Author:
    Nila122

URL:
    https://opengameart.org/content/lpc-clothes-and-hair

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    Original Walkcycle, Hurt, Slash, and Spellcast animations by Stephen Challener (AKA Redshrike) and Shooting and Thrusting animations by Johannes Sjölund

File(s):
    * Clothes00.zip

----------------------------------------

Title:
    LPC Expressions

Author:
    Dragontrainer

URL:
    https://opengameart.org/content/lpc-expressions-0

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    The sprites were originally made using Gaurav Mungal&#039;s Universal LPC Spritesheet Character Generator.

File(s):
    * LPC Expressions.png

----------------------------------------

Title:
    Blue potion

Author:
    Anzhel

Collaborators:
    Reid

URL:
    https://opengameart.org/content/blue-potion

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

Copyright/Attribution Notice:
    credits to Reid :)

File(s):
    * 23001.png

----------------------------------------

Title:
    LPC Brunet Mustache

Author:
    Nemisys

URL:
    https://opengameart.org/content/lpc-brunet-mustache

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Original Walkcycle, Hurt, Slash, and Spellcast animations by Stephen Challener (AKA Redshrike) and Shooting and Thrusting animations by Johannes Sjölund

File(s):
    * brunettemustache.png
    * character(brunettemustache).png

----------------------------------------

Title:
    LPC White Beard

Author:
    Nemisys

URL:
    https://opengameart.org/content/lpc-white-beard

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Original Walkcycle, Hurt, Slash, and Spellcast animations by Stephen Challener (AKA Redshrike) and Shooting and Thrusting animations by Johannes Sjölund

File(s):
    * whitebeard.png
    * character(whitebeard).png

----------------------------------------

Title:
    LPC Sign Post

Author:
    Nemisys

URL:
    https://opengameart.org/content/lpc-sign-post

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    Attribution to Nemisys

File(s):
    * signpost.png

----------------------------------------

Title:
    [LPC] Golem

Author:
    William.Thompsonj

Collaborators:
    Redshrike

URL:
    https://opengameart.org/content/lpc-golem

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    Attribute Stephen "Redshrike" Challener as graphic artist and William.Thompsonj as contributor. If reasonable link to this page or the OGA homepage.

File(s):
    * golem-walk.png
    * golem-atk.png
    * golem-die.png

----------------------------------------

Title:
    LPC Orange Trees

Author:
    Nemisys

URL:
    https://opengameart.org/content/lpc-orange-trees

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Original trees by C. Nilsson and recolored by William. Thompsonj while the table is based on art submitted by Daniel Eddeland.

File(s):
    * orangetrees.png
    * oranges.png

----------------------------------------

Title:
    Anime Portrait for LPC characters

Author:
    ZeNeRIA29

URL:
    https://opengameart.org/content/anime-portrait-for-lpc-characters

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

Copyright/Attribution Notice:
    1. Credit RPG Action as follows (linking is optional):
   "Portrait graphics created by PlayCraft" and link to 
  https://itunes.apple.com/us/app/playcraft/id1097458476?ls=1&mt=8
2. Credit the artist "ZeNeRIA29" as follows (optional):
   "Graphics designed by ZeNeRIA29" and link to 
   http://zeneria29.deviantart.com/

File(s):
    * LPCportrait1.png
    * LPCportrait2.png
    * LPCportrait3.png
    * LPCportrait4.png
    * LPCportrait5.png
    * LPCportrait6.png
    * LPCportrait7.png
    * LPCportrait8.png
    * LPCportrait9.png
    * LPCportrait10.png
    * LPCportrait11.png
    * LPCportrait22.png
    * portrait26.png
    * LPCportrait23.png
    * LPCportrait28.png
    * LPCportrait27.png
    * LPCportrait29.png

----------------------------------------

Title:
    Sara, Trevor, Puck  Anime Portrait and Expressions

Author:
    ZeNeRIA29

URL:
    https://opengameart.org/content/sara-trevor-puck-anime-portrait-and-expressions

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

Copyright/Attribution Notice:
    Credit RPG Action as follows (linking is optional): "Portrait graphics created by RPG Action" and link to http://peterlzy.wix.com/rpgaction 2. Credit the artist "ZeNeRIA29" as follows (optional): "Graphics designed by ZeNeRIA29" and link to http://zeneria29.deviantart.com/

File(s):
    * portrait21.png
    * portrait24.png
    * portrait25.png

----------------------------------------

Title:
    A bunch of rocks

Author:
    prushik

URL:
    https://opengameart.org/content/a-bunch-of-rocks

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

Copyright/Attribution Notice:
    No need to give me credit.

File(s):
    * rocks_CC0.png

----------------------------------------

Title:
    LPC Animated Water and waterfalls

Author:
    ZaPaper

URL:
    https://opengameart.org/content/lpc-animated-water-and-waterfalls

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

Copyright/Attribution Notice:
    Use the original LPC license and attribution.  Would be nice to also link to my RPG Game Maker.

File(s):
    * wateranimate2.png

----------------------------------------

Title:
    Bat sprite

Author:
    bagzie

URL:
    https://opengameart.org/content/bat-sprite

License(s):
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

File(s):
    * 32x32-bat-sprite.png

----------------------------------------

Title:
    Anime LPC Hair

Author:
    BlueVortexGames

URL:
    https://opengameart.org/content/anime-lpc-hair

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

Copyright/Attribution Notice:
    Credit is appreciated but not required, just credit me as Cobra Hubbard or BlueVortexGames

File(s):
    * messy.png

----------------------------------------

Title:
    LPC aprons

Author:
    Nila122

URL:
    https://opengameart.org/content/lpc-aprons

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

File(s):
    * Aprons.zip

----------------------------------------

Title:
    LPC long ears

Author:
    Nila122

URL:
    https://opengameart.org/content/lpc-long-ears

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

File(s):
    * Long ears.zip

----------------------------------------

Title:
    3 hairs for LPC

Author:
    Nila122

URL:
    https://opengameart.org/content/3-hairs-for-lpc

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

File(s):
    * Hair00.zip

----------------------------------------

Title:
    Bunny Rabbit LPC style for PixelFarm

Author:
    Redshrike

URL:
    https://opengameart.org/content/bunny-rabbit-lpc-style-for-pixelfarm

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

SPECIAL ATTRIBUTION INSTRUCTIONS:
    PixelFarm (https://bitbucket.org/tebruno99/pixelfarm)
Stephen "Redshrike" Challener

Copyright/Attribution Notice:
    Stephen Challener (Redshrike), commissioned by Tebruno99 and hosted by OpenGameArt.org

File(s):
    * bunnysheet5.png

----------------------------------------

Title:
    [LPC] Adobe Town Set

Author:
    bluecarrot16

URL:
    https://opengameart.org/content/lpc-adobe-town-set

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    "Adobe Town Set"
Artist: bluecarrot16
License: CC-BY 3.0 / GPL 3.0 / GPL 2.0 / OGA-BY 3.0

Please link to page on opengameart.

Based on "[LPC] Adobe Building Set" 
Artist: Sharm (Lanea Zimmerman)
Contributor: William.Thompsonj (William Thompson)
License: CC-BY 3.0 / GPL 3.0 / GPL 2.0 / OGA-BY 3.0
http://opengameart.org/content/lpc-adobe-building-set

File(s):
    * adobe-2.png

----------------------------------------

Title:
    [LPC] Signposts, graves, line cloths and scare crow

Author:
    Reemax

Collaborators:
    daneeklu, Nemisys

URL:
    https://opengameart.org/content/lpc-signposts-graves-line-cloths-and-scare-crow

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Attribution to Nemisys,Daniel Eddeland (http://opengameart.org/content/lpc-farming-tilesets-magic-animations-and-ui-elements) and Tuomo Untinen

File(s):
    * signpost-outsidestuff.png

----------------------------------------

Title:
    [LPC] Hanging signs

Author:
    Reemax

URL:
    https://opengameart.org/content/lpc-hanging-signs

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Please use the LPC attribution instructions for this work


File(s):
    * lpc-hanging-signs.png

----------------------------------------

Title:
    [LPC] House interior and decorations

Author:
    Reemax

Collaborators:
    Sharm, Hyptosis, daneeklu, William.Thompsonj, wulax, makrohn

URL:
    https://opengameart.org/content/lpc-house-interior-and-decorations

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    See credits.txt

File(s):
    * LPC_house_interior.zip

----------------------------------------

Title:
    Universal LPC Sprite Male 01

Author:
    Gaurav Munjal

URL:
    https://opengameart.org/content/universal-lpc-sprite-male-01

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Gaurav Munjal, Stephen Challener (Redshrike)

File(s):
    * universal-lpc-sprite_male_01_full.png
    * universal-lpc-sprite_male_01_walk-3frame.png
    * universal-lpc-sprite_male_01_walk-3frame.xcf.bz2

----------------------------------------

Title:
    [LPC] Horse

Author:
    bluecarrot16

URL:
    https://opengameart.org/content/lpc-horse

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    "[LPC] Horse"
Artist: bluecarrot16
License: CC-BY 3.0 / GPL 3.0 / GPL 2.0 / OGA-BY 3.0
Please link to opengameart: http://opengameart.org/content/lpc-horse

File(s):
    * horse.png

----------------------------------------

Title:
    [LPC] Forest tiles

Author:
    Reemax

Collaborators:
    Sharm, Hyptosis, Johann C, Beast, William.Thompsonj

URL:
    https://opengameart.org/content/lpc-forest-tiles

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    see credits.txt

File(s):
    * LPC_forest.zip

----------------------------------------

Title:
    [LPC] Cavern and ruin tiles

Author:
    Reemax

Collaborators:
    Sharm, Hyptosis, Johann C, HughSpectrum, Redshrike, William.Thompsonj, wulax

URL:
    https://opengameart.org/content/lpc-cavern-and-ruin-tiles

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    see credits.txt

File(s):
    * LPC_cavern_ruins.zip

----------------------------------------

Title:
    LPC modified art

Author:
    SuperMata

URL:
    https://opengameart.org/content/lpc-modified-art

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

Copyright/Attribution Notice:
    I found these images on the LPC guide website. My understanding is that the art is under the Creative Commons License. Please check the footer on the website.
http://lpc.opengameart.org/static/lpc-style-guide/assets.html

File(s):
    * LPC_Modifed_Art.zip

----------------------------------------

Title:
    [LPC] City outside

Author:
    Reemax

Collaborators:
    Sharm, daneeklu, Hyptosis, Johann C, William.Thompsonj, wulax

URL:
    https://opengameart.org/content/lpc-city-outside

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    see credits.txt


File(s):
    * LPC_city_outside.zip

----------------------------------------

Title:
    [LPC] Goat

Author:
    bluecarrot16

URL:
    https://opengameart.org/content/lpc-goat

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    "LPC Goat" by bluecarrot16. License: CC-BY 3.0 / GPL 2.0+
Based on "LPC Style Farm Animals" by Daniel Eddeland (daneeklu), commissioned by Thomas Bruno (tebruno99). License: CC-BY 3.0 / GPL 2+. http://opengameart.org/content/lpc-style-farm-animals

File(s):
    * goat.png

----------------------------------------

Title:
    [LPC] Turkey

Author:
    bluecarrot16

URL:
    https://opengameart.org/content/lpc-turkey

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    "LPC Turkey" by bluecarrot16. License: CC-BY 3.0 / OGA-BY 3.0 / GPL 2.0+ 

File(s):
    * turkey.png

----------------------------------------

Title:
    Bullet Collection 2 (M484 Games)

Author:
    Master484

URL:
    https://opengameart.org/content/bullet-collection-2-m484-games

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

File(s):
    * M484BulletCollection2.png

----------------------------------------

Title:
    [LPC] City inside

Author:
    Reemax

Collaborators:
    Sharm, HughSpectrum

URL:
    https://opengameart.org/content/lpc-city-inside

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    see credits.txt

File(s):
    * LPC_city_inside.zip

----------------------------------------

Title:
    [LPC] Wooden ship tiles

Author:
    Reemax

URL:
    https://opengameart.org/content/lpc-wooden-ship-tiles

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    Wooden ship tiles by Tuomo Untinen

File(s):
    * ship.png

----------------------------------------

Title:
    [LPC] Goblin - Full Sheet

Author:
    William.Thompsonj

Collaborators:
    Redshrike

URL:
    https://opengameart.org/content/lpc-goblin-full-sheet

License(s):
    * CC-BY 4.0 ( https://creativecommons.org/licenses/by/4.0/legalcode )
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )

Copyright/Attribution Notice:
    Provide a link to my profile. Attribute Stephen "Redshrike" Challener as graphic artist and William.Thompsonj as contributor.

File(s):
    * goblin-back-slash.gif
    * goblin.zip

----------------------------------------

Title:
    LPC Raven and Seagull

Author:
    Reemax

URL:
    https://opengameart.org/content/lpc-raven-and-seagull

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    Raven and Seagull by Tuomo Untinen

File(s):
    * birds.png

----------------------------------------

Title:
    [LPC] Rat, Cat and Dog

Author:
    Reemax

URL:
    https://opengameart.org/content/lpc-rat-cat-and-dog

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    Rat/Cat/Dog by Tuomo Untinen

File(s):
    * lpccatratdog.png

----------------------------------------

Title:
    [LPC] Fairy

Author:
    Reemax

URL:
    https://opengameart.org/content/lpc-fairy

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    Fairy by Tuomo Untinen

File(s):
    * fairy.xcf

----------------------------------------

Title:
    LPC Child Standing Template

Author:
    kheftel

URL:
    https://opengameart.org/content/lpc-child-standing-template

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * OGA-BY 3.0 ( http://static.opengameart.org/OGA-BY-3.0.txt )
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

File(s):
    * ChildStanding.png

----------------------------------------

Title:
    LPC compatible Ancient Greek Architecture

Author:
    TheraHedwig

URL:
    https://opengameart.org/content/lpc-compatible-ancient-greek-architecture

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Attribute to Wolthera van Hövell tot Westerflier, 

File(s):
    * LPC_Ancient_Greece_Architecture.png
    * LPC_compatible_Ancient_Greek_Architecture.zip

----------------------------------------

Title:
    [LPC] Glow Sword

Author:
    tskaufma

URL:
    https://opengameart.org/content/lpc-glow-sword

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

File(s):
    * glowsword.zip

----------------------------------------

Title:
    Another LPC style castle

Author:
    theidiotmachine

URL:
    https://opengameart.org/content/another-lpc-style-castle

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

File(s):
    * castle2.png

----------------------------------------

Title:
    LPC roman armor

Author:
    Nila122

URL:
    https://opengameart.org/content/lpc-roman-armor

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    Original cape by JaidynReiman, female plate chest by Matthew Krohn and male plate chest by Johannes Sjölund.

File(s):
    * Legion armor.zip

----------------------------------------

Title:
    LPC bandages

Author:
    Nila122

URL:
    https://opengameart.org/content/lpc-bandages

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

File(s):
    * Bandages.zip

----------------------------------------

Title:
    LPC masks

Author:
    Nila122

URL:
    https://opengameart.org/content/lpc-masks

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

File(s):
    * Masks.zip

----------------------------------------

Title:
    LPC Snowy Tree

Author:
    WolfMountainGames

Collaborators:
    Sharm

URL:
    https://opengameart.org/content/lpc-snowy-tree

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Sharm made this art for the LPC I just changed the colors to snow

File(s):
    * LPCsnowTrees.png

----------------------------------------

Title:
    LPC children walk animation

Author:
    Nila122

URL:
    https://opengameart.org/content/lpc-children-walk-animation

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    Original child sprite by kheftel.
Original elven ears by Makrohn.

File(s):
    * Children.zip

----------------------------------------

Title:
    [LPC] Items and game effects

Author:
    Reemax

Collaborators:
    Sharm, ETTiNGRiNDER, wulax, Nila122, daneeklu, JaidynReiman, pennomi, laetissima, makrohn, Jetrel

URL:
    https://opengameart.org/content/lpc-items-and-game-effects

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )
    * GPL 2.0 ( http://www.gnu.org/licenses/old-licenses/gpl-2.0.html )

Copyright/Attribution Notice:
    See credits.txt

File(s):
    * ItemsAndEffects.zip

----------------------------------------

Title:
    LPC castle

Author:
    Evert

Collaborators:
    Sharm, Xenodora

URL:
    https://opengameart.org/content/lpc-castle

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

Copyright/Attribution Notice:
    Credit me and link back here.
Credit Lanea Zimmerman (Sharm) for the use of the base LPC wood elements and Xenodora for the rounded towers.

File(s):
    * lpc_castle.png

----------------------------------------

Title:
    Cute dungeon LPC edit

Author:
    Evert

Collaborators:
    Buch, Sharm, William.Thompsonj

URL:
    https://opengameart.org/content/cute-dungeon-lpc-edit

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

Copyright/Attribution Notice:
    Credit Buch for the original version of this set and link to his profile page.
The portcullis are based on the iron bars in http://opengameart.org/content/lpc-dungeon-elements, by Sharm as contributed by William.Thompsonj.

File(s):
    * cute_lpc.png

----------------------------------------

Title:
    LPC cliffs/mountains with grass top and more!

Author:
    Evert

Collaborators:
    Sharm, daneeklu

URL:
    https://opengameart.org/content/lpc-cliffsmountains-with-grass-top-and-more

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    Credit the original author Lanea Zimmerman ("Sharm") for these assets. Additionally, credit Daniel Eddeland and Wiliam Thompson for the sand.
There is no need to credit me.

File(s):
    * LPC_cliffs_grass.png
    * LPC_cliffs_ddirt.png
    * LPC_cliffs_snow.png
    * LPC_cliffs_sand.png

----------------------------------------

Title:
    [LPC] Misc animations and items

Author:
    ak-blanc

URL:
    https://opengameart.org/content/lpc-misc-animations-and-items

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    I don&#039;t need any attribution but credit Sharm if you&#039;re using the animations and Stephen Challener and Thane Brimhall if you&#039;re using the top-hat character.

File(s):
    * chest_rounded.png
    * chest_squared.png
    * gate.png
    * courage_drop.png
    * host_mini.png
    * key0.png
    * key1.png
    * key2.png
    * key3.png
    * key4.png

----------------------------------------

Title:
    LPC Caves

Author:
    sharm

URL:
    https://opengameart.org/content/lpc-caves

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

Copyright/Attribution Notice:
    I believe the original author is:
Lanea Zimmerman (AKA Sharm) 
so please use that as the attibution

File(s):
    * caves.png

----------------------------------------

Title:
    LPC Additional Mountain and Cave Opening Tiles

Author:
    sharm

URL:
    https://opengameart.org/content/lpc-additional-mountain-and-cave-opening-tiles

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

Copyright/Attribution Notice:
    I believe the original author is: Lanea Zimmerman (AKA Sharm) so please use that as the attibution

File(s):
    * cave-opening.png

----------------------------------------

Title:
    [LPC] Thatched-roof Cottage

Author:
    bluecarrot16

URL:
    https://opengameart.org/content/lpc-thatched-roof-cottage

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    "LPC Thatched-roof Cottage" by bluecarrot16. License: CC-BY-SA 3.0 / GPL 3.0+

Based on:
"LPC Base Assets" by Lanea Zimmerman (AKA Sharm) and Daniel Armstrong (AKA HughSpectrum). 
License: CC-BY-SA 3.0 / GPL 3.0+. 
http://opengameart.org/content/liberated-pixel-cup-lpc-base-assets-sprites-map-tiles
"LPC art entry" by Casper Nilsson. 
License: CC-BY-SA 3.0 / GPL 3.0+. 
http://opengameart.org/content/lpc-cnilsson

File(s):
    * cottage.png
    * thatched-roof.png

----------------------------------------

Title:
    [LPC] Windows & Doors

Author:
    bluecarrot16

URL:
    https://opengameart.org/content/lpc-windows-doors

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )
    * GPL 3.0 ( http://www.gnu.org/licenses/gpl-3.0.html )

Copyright/Attribution Notice:
    "LPC Windows & Doors" by bluecarrot16, Lanea Zimmerman (Sharm) and Daniel Armstrong (HughSpectrum), Casper Nilsson, Anamaris, Krusmira, Keith Karnage, Guido Bos, and Talosaurus. License: CC-BY-SA 3.0 / GPL 3.0+ Please link back to this page on OpenGameArt: https://opengameart.org/content/lpc-windows-doors  . See CREDITS-windows-doors.txt. You MUST include all information in this file when distributing these images!

File(s):
    * windows.png
    * doors.png
    * lpc-windows-doors-v2.zip

----------------------------------------

Title:
    [LPC] Character Skintone Rework

Author:
    ElizaWy

URL:
    https://opengameart.org/content/lpc-character-skintone-rework

License(s):
    * CC-BY-SA 3.0 ( http://creativecommons.org/licenses/by-sa/3.0/legalcode )

Copyright/Attribution Notice:
    Babies are my original work. Original adult walkcycles done by Redshrike. Standing child template by Kheftel. Child walk cycle by Nila122.

File(s):
    * ElizasLpcSkintones.zip

----------------------------------------

Title:
    LPC explosions

Author:
    theidiotmachine

URL:
    https://opengameart.org/content/lpc-explosions

License(s):
    * CC0 ( http://creativecommons.org/publicdomain/zero/1.0/legalcode )

File(s):
    * Pix.zip

----------------------------------------

Title:
    LPC Anime Portrait #2

Author:
    ZaPaper

Collaborators:
    ZaPaper

URL:
    https://opengameart.org/content/lpc-anime-portrait-2

License(s):
    * CC-BY 3.0 ( http://creativecommons.org/licenses/by/3.0/legalcode )

Copyright/Attribution Notice:
    1. Credit RPG Action as follows (linking is optional): "Portrait graphics created by PlayCraft" and link to https://itunes.apple.com/us/app/playcraft/id1097458476?ls=1&mt=8 2. Credit the artist "ZeNeRIA29" as follows (optional): "Graphics designed by ZeNeRIA29" and link to http://zeneria29.deviantart.com/

File(s):
    * LPCportrait30.png
    * LPCportrait31.png
    * LPCportrait32.png
    * LPCportrait33.png

----------------------------------------
